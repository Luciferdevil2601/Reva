...
export function PreviewPanel(){
  ...
  const data=useMemo(()=>s.optimizedData||parseResumeText(s.resumeText),[s.optimizedData,s.resumeText]);
  return <section className="previewShell"><div className="previewTop"><select value={s.selectedTemplate} onChange={e=>s.setTemplate(e.target.value as TemplateId)}>{templates.map(t=><option key={t} value={t}>{t}</option>)}</select><span>A4</span><span>Zoom: 72%</span><button className="btn secondary" onClick={()=>window.print()}>Print Preview</button></div><div className="paperBg"><ResumeTemplate data={data} template={s.selectedTemplate}/></div></section>;
}
