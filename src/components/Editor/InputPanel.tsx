...
export function InputPanel(){
  ...
  return <aside className="inputPanel"><div className="formScroll"><div className="field"><label>Job Description <small>{s.jdText.length}/4000</small></label><textarea rows={8} value={s.jdText} onChange={e=>s.setJD(e.target.value)} placeholder="Paste the full job description here..."/></div><div><b>Upload Old Resume</b><button type="button" className={`uploadBox ${resumeFile?'parsed':''}`} onClick={()=>fileRef.current?.click()} disabled={uploading}><input ref={fileRef} type="file" accept=".pdf,.docx,.txt,application/pdf,application/vnd.openxmlformats-officedocument.wordprocessingml.document,text/plain" hidden onChange={e=>e.target.files?.[0]&&uploadResume(e.target.files[0])}/>{uploading?<Loader2 className="spin"/>:<FileUp/>}<span>{uploading?'Parsing resume...':resumeFile?`Parsed: ${resumeFile.name}`:'Click to upload PDF, DOCX, or TXT resume'}</span>{resumeFile&&<small>{Math.round(resumeFile.size/1024)} KB parsed and ready</small>}</button>{uploadError&&<p className="errorText">{uploadError}</p>}{s.resumeText&&<details className="parsedText"><summary>View parsed resume text</summary><textarea rows={8} value={s.resumeText} onChange={e=>s.setResume(e.target.value)}/></details>}</div><div className="card"><Upload size={18}/><p><b>Workflow:</b> paste JD, upload old resume, optimize directly or analyze first, download PDF.</p></div><TemplateSelector/></div><div className="stickyActions"><button className="btn" onClick={s.analyze} disabled={s.isAnalyzing||s.isOptimizing||!s.jdText.trim()||!s.resumeText.trim()}>{s.isAnalyzing?'Analyzing...':'Analyze ATS Score'}</button><button className="btn" onClick={s.optimize} disabled={s.isOptimizing||s.isAnalyzing||!s.jdText.trim()||!s.resumeText.trim()}>{s.isOptimizing?'Optimizing...':'Optimize Resume'}</button><button className="btn secondary" onClick={s.downloadPDF} disabled={s.step!=='optimized'||s.isDownloading}>{s.isDownloading?'Preparing PDF...':'Download PDF'}</button>{s.error&&<p style={{color:'var(--red)'}}>{s.error}</p>}</div></aside>}
